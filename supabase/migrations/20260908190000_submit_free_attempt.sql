-- Client-reported free-quiz attempts (unverified, anti-cheat safe).
-- Records QuizSession submissions server-side with verified=false so that
-- /admin/attempts updates continuously again. Spam-guarded:
--   * attempt attributed to auth.uid(), never to a client-supplied user id
--   * min 10s between two submissions of the same user
--   * max 100 submissions per user per hour
--   * strict range checks on every client-supplied number
--   * (user_id, history_id) dedupe: retries/double-submits never duplicate rows

create unique index if not exists practice_attempts_user_history_idx
  on public.practice_attempts (user_id, history_id);
...
begin
  ...
  select count(*) into v_hour_count from public.practice_attempts
  ...
  insert into public.practice_attempts
    (user_id, history_id, exam_id, subject_id, title, mode, score, correct, total, accuracy, duration_seconds, retry_of, retry_number, completed_at, verified, source)
  values
