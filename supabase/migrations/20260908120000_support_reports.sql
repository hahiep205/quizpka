create table if not exists public.support_reports (
  ...
  type text not null default 'report' check (type in ('report','contribute','feedback')),
...
begin
  ...
  insert into public.support_reports(user_id, type, subject, description, page_url) values (auth.uid(), p_type, trim(p_subject), trim(p_description), nullif(trim(p_page_url), '')) returning id into report_id;
  return report_id;
end; $$;
