...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const input = await req.json().catch(() => null) as { examId?: unknown } | null
    const examId = typeof input?.examId === "string" ? input.examId : ""
    const objectPath = examFiles[examId]
    if (!objectPath) return new Response(JSON.stringify({ error: "Unknown exam" }), { status: 400, headers: cors(req) })
