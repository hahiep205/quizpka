...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (rawProductId !== "dsai101" && rawProductId !== "idsai101" && rawProductId !== "sqa101" && rawProductId !== "sec301" && rawProductId !== "mar101" && rawProductId !== "mac102" && rawProductId !== "oit101" && rawProductId !== "fin101" && rawProductId !== "civ101") return Response.json({ error: "Product unavailable" }, { status: 400, headers: cors(req) })
    ...
    let orderId = pending?.order_id ?? `${productId === "sqa101" ? "SQA" : productId === "sec301" ? "SEC" : productId === "idsai101" ? "IDSAI" : productId === "mar101" ? "MAR" : productId === "mac102" ? "MAC" : productId === "oit101" ? "OIT" : productId === "fin101" ? "FIN" : productId === "civ101" ? "CIV" : "DSAI"}-${crypto.randomUUID().replaceAll("-", "").slice(0, 20).toUpperCase()}`
